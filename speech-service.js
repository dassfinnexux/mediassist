...
class SpeechService {
  ...
  setLanguage(langCode) {
    ...
    this.speechConfig.speechRecognitionLanguage =
      CONFIG.LANGUAGES[langCode].speechCode;

    if (CONFIG.DEBUG)
      console.log(`🌐 Language set: ${CONFIG.LANGUAGES[langCode].name}`);
  }

  setVoiceGender(gender) {
    if (gender !== "male" && gender !== "female") return;
    this.voiceGender = gender;
    if (CONFIG.DEBUG) console.log(`🔊 Voice gender: ${gender}`);
  }
  ...
  async startRecognition(onResult, onError, onStart, onEnd) {
    if (this.isListening) {
      CONFIG.DEBUG && console.warn("Already listening");
      return;
    }
    ...
    try {
      ...
      this.recognizer.sessionStarted = () =>
        CONFIG.DEBUG && console.log("🎙️ STT session started");
      this.recognizer.sessionStopped = () =>
        CONFIG.DEBUG && console.log("🛑 STT session stopped");

      // cancel for both modes
      ...
      if (CONFIG.CONTINUOUS_RECOGNITION) {
        ...
        // Single utterance mode: ONLY recognizeOnceAsync (no recognized handler)
  ...
  async speak(text, langCode = null) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        CONFIG.DEBUG &&
          console.log(`🔊 Speaking (${langConfig.name}): "${text.slice(0, 50)}..."`);
        ...
        this.synthesizer.speakTextAsync(
          ...
          (result) => {
            ...
            else reject(new Error(result.errorDetails || "Speech synthesis failed"));
          },
          (err) => {
            try {
              this.synthesizer?.close();
